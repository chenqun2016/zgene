...
class CusDateUtils {
  ...
  ///format:"yyyy年MM月dd hh:mm:ss"  "yyy?MM?dd  hh?MM?dd" "yyyy:MM:dd"......
  ///结果： 2019?08?04  02?08?02
  static getFormatDataS({timeSamp: int, format: String}) {
    var dataFormat = new DateFormat(format);
  ...
  ///使用:    List<String> mdata=CusDateUtils.instance.getTimeBettwenStartTimeAndEnd(startTime:"2019-07-11",endTime:"2019-08-29",format:"yyyy年MM月dd");
  ...
  ///使用：CusDateUtils.instance.getTimeStartTimeAndEnd(startTime:"2019-07-11",dayNumber:10,format:"yyyy年MM月dd");
  ...
  ///dayNumber：从startTime往后面多少天你需要输出
  ///format:获取到的日期格式。"yyyy年MM月dd" "yyyy-MM-dd" "yyyy年" "yyyy年MM月" "yyyy年\nMM月dd"  等等
  ///使用：CusDateUtils.instance.getOldDate(startTime:"2019-07-11",dayNumber:10,format:"yyyy年MM月dd");
  ///结果:2019年07月21
  static String getOldDateStr(
      {startTime: String, dayNumber: int, format: String}) {
    var date;
    //记录往后每一天的时间搓，用来和最后一天到做对比。这样就能知道什么时候停止了。
    DateTime startData = DateTime.parse(startTime);
  ...
  static List<String> getBeforeDayAndBeforeHour() {
    ...
    /// 更改当前时间的时间格式
    time.add(CusDateUtils.getFormatDataString(now, 'yyyy-MM-dd'));

    /// 当前时间的前一天
    String _theDayBefore = CusDateUtils.getFormatDataString(
        now.add(new Duration(days: -1)), 'yyyy-MM-dd');
    time.add(_theDayBefore);

    /// 当前时间的前一小时
    String _theHourBefore = CusDateUtils.getFormatDataString(
        now.add(new Duration(hours: -1)), 'HH');
    time.add(_theHourBefore);

    /// 当前时间的小时
    String _hour = CusDateUtils.getFormatDataString(now, "HH");
    time.add(_hour);
